...
def create_unidades_administrativas(atualizacao = False, nome_json = '2018'):
...
def create_tipo_local(atualizacao = False, nome_json = '2018'):
    tipo_local = {}
    colunas_tipo_local = ['DESC_TIPOLOCAL']

    armazena_valores.armazena_valores(tipo_local, colunas_tipo_local, 'tipo_local', atualizacao,True, nome_json)


def create_pessoas(atualizacao = False, nome_json = '2018'):
    pessoas = {}
    colunas_pessoas = ['TIPO_PESSOA', 'SEXO_PESSOA', 'IDADE_PESSOA', 'DATA_NASCIMENTO_PESSOA', 'COR_PELE', 'PROFISSAO']

    armazena_valores.armazena_valores(pessoas, colunas_pessoas, 'pessoas', atualizacao, False, nome_json)


def create_ocorrencias(atualizacao = False, nome_json = '2018'):
    ocorrencias = {}
    colunas_ocorrencias = ['NUM_BO', 'DATAHORA_REGISTRO_BO', 'ANO_BO']

    armazena_valores.armazena_valores(ocorrencias, colunas_ocorrencias, 'ocorrencias', atualizacao, True, nome_json)


def create_locais(atualizacao = False, nome_json = '2018'):
    locais = {}
    colunas_locais = ['LOGRADOURO', 'NUMERO_LOGRADOURO', 'LATITUDE', 'LONGITUDE']

    armazena_valores.armazena_valores(locais, colunas_locais, 'locais', atualizacao, True, nome_json)


def create_homicidios(atualizacao = False, nome_json = '2018'):
    homicidios = {}
...
def create_delegacias_especializadas(atualizacao = False, nome_json = '2018'):
