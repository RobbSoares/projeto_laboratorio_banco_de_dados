...
query = open('tag_titulo/query.sql', 'w')
genre_query = open('genero_titulo/genre.sql', 'w')
network_query = open('network_titulo/network.sql', 'w')
exibicao_query = open('exibicao_titulo/exibicao_query.sql', 'w')
elenco_query = open('elenco_titulo/elenco_query.sql', 'w')

with open('tag_titulo/top100_kdrama.json', encoding='utf-8') as fh:
    data = pd.read_json(fh)

with open('tag_titulo/tags.txt', encoding='utf-8') as tags:
    tag_names = tags.readlines()

with open('tag_titulo/titulos.txt', 'r') as titles:
    title_names = titles.readlines()

with open('genero_titulo/genero.txt', 'r') as genres:
    genre_names = genres.readlines()

with open('network_titulo/network.txt', 'r') as network:
    network_names = network.readlines()

with open('exibicao_titulo/exibicao.txt', 'r') as exibicao:
    exibicao_day = exibicao.readlines()

with open('elenco_titulo/elenco.txt', 'r') as elenco:
...
network_dict = data['Network'].to_dict()
